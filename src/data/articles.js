...
// Flattened articles array for Article.jsx slug lookup
export const allArticles = Object.values(categoryData).flatMap(category =>
  [...category.featured, ...category.latest].map(item => ({
    slug: item.slug,
    title: item.title,
    content: item.teaser,
    category: Object.keys(categoryData).find(key => categoryData[key] === category)
  }))
);
